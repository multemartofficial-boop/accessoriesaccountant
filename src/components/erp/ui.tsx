    ...
    <div className="mb-5 grid grid-cols-2 gap-3 xl:grid-cols-4">
      {items.map((item, index) => {
        ...
        return <div key={item.label} className={cn("relative min-w-0 overflow-hidden rounded-lg border border-l-[3px] bg-card px-3 py-3.5 shadow-card sm:px-4 sm:py-4", accent)}>
          ...
          <div className="mt-2 break-words text-lg font-bold leading-tight text-foreground sm:text-[clamp(1.35rem,2vw,1.7rem)]">{item.value}</div>
...
export function MiniStats({ items }: { items: { label: string; value: string; detail?: string; tone?: MetricTone }[] }) {
  return <div className="grid grid-cols-2 gap-3 xl:grid-cols-3">{items.map((item) => <div key={item.label} className="min-w-0 rounded-lg border bg-card p-3.5 shadow-card sm:p-4"><div className="text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground">{item.label}</div><div className={cn("mt-1.5 break-words text-lg font-bold sm:text-xl", item.tone === "down" && "text-destructive", item.tone === "up" && "text-success")}>{item.value}</div>{item.detail && <div className="mt-1 text-[10px] leading-4 text-muted-foreground">{item.detail}</div>}</div>)}</div>;
}
